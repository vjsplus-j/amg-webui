...
3. 五大业务模块：`login` · `users` · `orders` · `content` · `settings`；biz 调试页在 `example/pages/biz/`。行业套件在 `packages/components/industry/`（catalog `industry` **47** 件，含码制 Barcode/Qrcode/MatrixCode/OcrScan）。
