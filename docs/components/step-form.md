...
StepForm 分步表单：面向企业场景的 Form 组件（成熟度 rc）。
...
| `steps` | `StepFormStep[]` | `() => []` | steps 列表数据 |
| `stepData` | `Record<string, Record<string, unknown>>` | `() => ({})` | stepData 字符串 |
...
| `update:stepData` | `value: Record<string, Record<string, unknown>>` | `stepData` 更新时触发（v-model） |
...
| metadata | `component-metadata/StepForm.json` |
| API extract | `generated/component-api/StepForm.json` |

> 完整 Demo 见 `example/demos/StepForm`（example 本地调试，不上线）。
