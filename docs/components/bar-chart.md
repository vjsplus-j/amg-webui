...
BarChart：面向企业场景的 Charts 组件（成熟度 rc）。
...
- 状态：`N/A`
- tests/unit/hardening/real-mount-remaining.spec.ts BarChart non-interactive display — keyboard N/A
...
- 状态：`PASS`
- tests/unit/hardening/real-mount-remaining.spec.ts BarChart uses package styles / semantic tokens gate

## RTL

- 状态：`N/A`
- tests/unit/hardening/real-mount-remaining.spec.ts BarChart RTL covered by theme/dir provider

## SSR

- 状态：`PASS`
- tests/unit/hardening/real-mount-remaining.spec.ts BarChart client mount OK; no required browser-only top-level in package gate
...
| metadata | `component-metadata/BarChart.json` |
| API extract | `generated/component-api/BarChart.json` |

> 完整 Demo 见 `example/demos/BarChart`（example 本地调试，不上线）。
