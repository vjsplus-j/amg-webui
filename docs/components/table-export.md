...
TableExport：面向企业场景的 Table 组件（成熟度 rc）。
...
- 状态：`PASS`
- tests/unit/hardening/real-mount-remaining.spec.ts TableExport uses package styles / semantic tokens gate

## RTL

- 状态：`N/A`
- tests/unit/hardening/real-mount-remaining.spec.ts TableExport RTL covered by theme/dir provider

## SSR

- 状态：`PASS`
- tests/unit/hardening/real-mount-remaining.spec.ts TableExport client mount OK; no required browser-only top-level in package gate
...
| metadata | `component-metadata/TableExport.json` |
| API extract | `generated/component-api/TableExport.json` |

> 完整 Demo 见 `example/demos/TableExport`（example 本地调试，不上线）。
