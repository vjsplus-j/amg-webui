...
VcrStorageDashboard 为 **RC** 公共组件（Contract maturity=`rc`）。本文档由 `generate-vitepress-api.mjs` 从 `generated/component-api` 生成。

## 概览

VcrStorageDashboard 当前成熟度为 **RC**；完整交互见本地 example。
...
| `refresh` | `void` | — |
| `export` | `volumes: VcrStorageVolume[], summary: VcrStorageSummary` | — |
...
> 完整 Demo 见 `example/demos/VcrStorageDashboard`。对外 docs 为 API 导向页面；交互预览见本地 example（不上线）。
