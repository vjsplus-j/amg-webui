...
Machine404：面向企业场景的 Special 组件（成熟度 rc）。
...
- 状态：`N/A`
- tests/unit/hardening/real-mount-remaining.spec.ts Machine404 non-interactive display — keyboard N/A
...
- 状态：`PASS`
- tests/unit/hardening/real-mount-remaining.spec.ts Machine404 uses package styles / semantic tokens gate

## RTL

- 状态：`N/A`
- tests/unit/hardening/real-mount-remaining.spec.ts Machine404 RTL covered by theme/dir provider

## SSR

- 状态：`PASS`
- tests/unit/hardening/real-mount-remaining.spec.ts Machine404 client mount OK; no required browser-only top-level in package gate
...
| metadata | `component-metadata/Machine404.json` |
| API extract | `generated/component-api/Machine404.json` |

> 完整 Demo 见 `example/demos/Machine404`（example 本地调试，不上线）。
