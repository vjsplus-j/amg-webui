...
/** Single entry for RichText HTML sanitization (always pass the same options). */
export function sanitizeRichTextValue(
  html: string,
  enabled: boolean,
  options?: SanitizeHtmlOptions
): string {
  return enabled ? sanitizeHtml(html || '', options) : html || ''
}

export function readSanitizedHtml(
  editor: HTMLElement,
  enabled: boolean,
  options?: SanitizeHtmlOptions
): string {
  return sanitizeRichTextValue(editor.innerHTML, enabled, options)
}

export function writeSanitizedHtml(
  editor: HTMLElement,
  html: string,
  enabled: boolean,
  options?: SanitizeHtmlOptions
): void {
  const next = sanitizeRichTextValue(html, enabled, options)
