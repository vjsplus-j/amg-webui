...
import { sanitizeUrl } from '@amg-webui/security'
...
/** All HTML sanitize paths share the same whitelist options. */
const sanitize = (value: string) =>
  sanitizeRichTextValue(value, props.sanitize, props.sanitizeOptions)
...
const syncFromProp = () => {
  ...
  const incoming = sanitize(props.modelValue || '')
...
const pushFromEditor = (trackType = 'input', coalesce = false) => {
  const el = editorRef.value
  if (!el) return
  const html = readSanitizedHtml(el, props.sanitize, props.sanitizeOptions)
...
const onPaste = (event: ClipboardEvent) => {
  ...
  const safe = html ? sanitize(html) : sanitize(`<p>${text}</p>`)
...
onMounted(() => {
  syncFromProp()
  const initial = sanitize(props.modelValue || '')
