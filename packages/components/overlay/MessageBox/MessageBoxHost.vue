...
const overlay = useOverlay({
  ...
  autoFocus: false,
  onEscape: () => {
    if (closeOnEscape.value) void close('escape')
  }
})

const descriptionId = computed(() => `${overlay.titleId}-description`)
...
watch(
  () => props.visible,
  (v) => {
    if (v) void nextTick(focusTarget)
  },
  { immediate: true }
)
          ...
          <header class="vp-message-box__header">
            <div class="vp-message-box__lead">
              ...
              <div class="vp-message-box__titles">
                <h3 :id="overlay.titleId" class="vp-message-box__title">
