...
const tourVisible = computed(() => props.open && props.steps.length > 0)
...
const overlay = useOverlay({
  ...
  restoreFocus: true,
  closeOnEscape: true,
  onEscape: () => closeTour('close'),
  autoFocus: false
})

const tourLayerStyle = computed(() => {
...
<template>
  <Teleport :to="overlay.teleportTo.value ?? 'body'">
    <Transition name="vp-tour-fade">
      <div
        v-if="open && steps.length"
        ref="panelRef"
        class="vp-tour"
        :style="tourLayerStyle"
        data-component="Tour"
      >
        <div
          v-if="mask"
