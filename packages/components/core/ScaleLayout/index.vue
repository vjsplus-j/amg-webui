...
const emit = defineEmits<ScaleLayoutEmits>();
...
watch(
  resolvedScale,
  (scale) => emit("scaleChange", scale, { ...hostSize.value }),
  { immediate: true },
);

const hasDesign = computed(
  () => designW.value != null && designH.value != null,
);
...
<template>
  <div
    ...
    :class="rootClass"
    :style="rootStyle"
    data-component="ScaleLayout"
    :role="ariaLabel ? 'region' : 'group'"
