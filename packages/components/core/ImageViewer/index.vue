...
const overlay = useOverlay({
  ...
  restoreFocus: true,
  closeOnEscape: true,
  teleportTo: () => (props.teleported ? 'body' : undefined),
  onEscape: () => close()
})
...
watch(
  ...
  (visible) => {
    ...
    if (!doc) return
    if (visible) doc.addEventListener('keydown', onNavKeydown)
    else doc.removeEventListener('keydown', onNavKeydown)
  },
  { immediate: true }
)
...
onUnmounted(() => {
  if (!isClient) return
  getDocument()?.removeEventListener('keydown', onNavKeydown)
})
</script>

<template>
  <Teleport :to="overlay.teleportTo.value ?? 'body'" :disabled="!teleported">
