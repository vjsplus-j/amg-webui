...
const mergedConfig = computed<ConfigProviderResolvedConfig>(() => {
  ...
  return {
    ...
    overlayTeleportTo: props.overlayTeleportTo ?? parent.overlayTeleportTo,
    overlayRuntime: props.overlayRuntime ?? parent.overlayRuntime,
...
watch(
  ...
  (config) => {
    ...
    const patch: {
      zIndexBase?: number
      namespace?: string
      teleportTo?: typeof config.overlayTeleportTo
    } = {}
    if (config.zIndex !== undefined) patch.zIndexBase = config.zIndex
    if (config.namespace !== undefined) patch.namespace = config.namespace
    if (config.overlayTeleportTo !== undefined) {
