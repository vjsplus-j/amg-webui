...
import type { ButtonGlobalConfig } from '@amg-webui/core/Button/config'
...
import type { OverlayRuntimeApi, TeleportTarget } from '@amg-webui/runtime'
...
export interface ConfigProviderProps extends BaseProps {
  ...
  zIndex?: number
  namespace?: string
  /** Default Teleport target for Overlay runtime (`useOverlay`). */
  overlayTeleportTo?: TeleportTarget
  /** Scoped Overlay runtime (micro-FE). When set, provided to descendants via OVERLAY_RUNTIME_KEY. */
