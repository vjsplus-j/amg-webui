...
import type { ErrorModalProps, ErrorModalEmits } from './types'
import './style.scss'
...
<template>
  <Teleport to="body">
    <Transition name="vp-error-modal-fade">
      <div v-if="visible" class="vp-error-modal-overlay" @click="onOverlay">
        <div :class="rootClass" :style="style" role="alertdialog" @click.stop>
          <header class="vp-error-modal__header">
            <h3 class="vp-error-modal__title">
              <slot name="title">{{ title }}</slot>
            </h3>
            <button
              v-if="closable"
              type="button"
              class="vp-error-modal__close"
              :aria-label="closeLabel"
              @click="close"
            >×</button>
          </header>
          <div class="vp-error-modal__body">
            <slot>{{ message }}</slot>
          </div>
          <footer class="vp-error-modal__footer">
            <slot name="footer">
              <button type="button" class="vp-error-modal__btn vp-error-modal__btn--ghost" @click="close">
                {{ cancelText }}
              </button>
              <button type="button" class="vp-error-modal__btn vp-error-modal__btn--primary" @click="confirm">
                {{ confirmText }}
              </button>
            </slot>
          </footer>
