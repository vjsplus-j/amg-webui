...
<template>
  <section :class="['vp-drag-ruler', 'vp-drag-ruler__panel', props.class]" :style="style" role="region" aria-labelledby="vp-drag-ruler-title" data-component="DragRuler">
    <header class="vp-drag-ruler__header">
      <h3 id="vp-drag-ruler-title" class="vp-drag-ruler__title">{{ titleText }}</h3>
      <div class="vp-drag-ruler__status" role="status"></div>
    </header>
    <div v-if="loading" class="vp-drag-ruler__loading" role="status">{{ t(LocaleKeys.common.loading) }}</div>
    <div v-else class="vp-drag-ruler__body">
      <div class="vp-drag-ruler__h"><span v-for="tick in ticks" :key="'h'+tick" class="vp-drag-ruler__tick">{{ tick }}</span></div>
